...
(
  ...
  docker rmi -f kndoll/kstock-frontend:latest kndoll/kstock-server:latest kndoll/kstock-worker:latest postgres:15-alpine redis:7-alpine rabbitmq:3-management-alpine confluentinc/cp-kafka:7.5.0 chromadb/chroma:0.5.11 alpine containrrr/watchtower >/dev/null 2>&1
) &
show_spinner $! "K-STOCK LIVE 컨테이너, 볼륨, 이미지 제거 중..."
wait $!
rm -f ".env"
echo -e " ${GREEN}✔ K-STOCK LIVE와 관련된 모든 시스템 및 데이터가 완벽히 제거되었습니다.${NC}"
echo ""
